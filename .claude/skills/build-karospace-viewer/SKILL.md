...
| `--cell-annotations` | **Expose EVERY clustering / niche / domain annotation, not a curated subset** — users switch between them, so a missed resolution is a missed view. Sweep obs and pass them ALL: clustering at any resolution (`leiden`, `louvain`, `kmeans`, `snn`, and resolution-suffixed variants like `leiden_0_2`…`leiden_4_0`); spatial domains/niches at any k (`CellCharter`, `niche`, `domain`, e.g. `CellCharter_6`…`CellCharter_30`); region/segmentation labels (polygon labels, manual regions). Include a column if its name matches one of these families **or** it's categorical with cardinality ~2–300 that isn't an experimental variable (→ `--section-metadata`), an ID (cardinality ≈ cell count, e.g. `cell_id`), or a QC metric. **When unsure, include it** — an extra dropdown is cheaper than a missing annotation. Never expose ID columns or per-cell continuous QC numerics. |
